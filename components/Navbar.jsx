  ...
  return (
    <nav className="sticky w-full top-0 z-40 bg-rose-900">
      <div className="relative mx-auto flex h-14 max-w-7xl items-center justify-between px-2 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-1 items-center justify-center min-[350px]:justify-around sm:items-stretch ">
          {/* Logo */}
          <Link className="flex flex-shrink-0 items-center" href="/">
            <Image src={WorldCupLogo} alt="World Cup Logo" unoptimized width={50} height={50} className="block h-8 w-auto" />
            <span className="mx-3 hidden cursor-pointer text-xl md:text-2xl font-semibold tracking-tight text-zinc-100 hover:text-white min-[400px]:block  ">
              World Cup 2022
            </span>
          </Link>
          ...
          <div className="flex justify-center items-center gap-1">
            ...
            />
            <div
              className={`cursor-pointer transition-transform duration-300 transform ${showAllCountries ? "" : "rotate-90"}`}
              onClick={handleShowAllCountries}
            >
              <Image
                src={arrowDown}
              ...
              {navigation.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {item.subMenu && (
                      <div className="absolute left-0 top-full z-10 hidden w-32 transform whitespace-nowrap  bg-rose-700 shadow-lg group-hover:block  ">
                        {item.subMenu.map((subItem) => (
                          <div className="group/sec relative" key={subItem.name}>
                            <Link
                              ...
                              className="block w-full rounded-none px-4 py-2 text-sm text-white  hover:bg-rose-900    "
                            >
                              {subItem.name}
                            </Link>
                            {subItem.subMenu && (
                              <div className="absolute top-0 left-full z-10 hidden w-32 transform -translate-x-1 whitespace-nowrap rounded-lg rounded-l-none hover:rounded-tl-none bg-rose-900  shadow-lg group-hover/sec:block group-hover/sec:translate-x-0">
